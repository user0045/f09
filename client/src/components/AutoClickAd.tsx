...
const AutoClickAd: React.FC<AutoClickAdProps> = ({ pageType }) => {
  ...
  useEffect(() => {
    ...
    const startAutoClickSystem = () => {
      ...
      // Initial click after 5 seconds (only once per page load)
      if (!hasInitialClickedRef.current) {
        timersRef.current.initial = setTimeout(async () => {
          console.log('Executing initial enhanced auto-click (5 seconds after page load)');
          try {
            ...
            if (success) {
              ...
              console.log('Initial enhanced auto-click failed - will retry in 3 seconds');
